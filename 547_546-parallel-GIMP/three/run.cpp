...
bool myPatch::afterStep(){
   ...
   for(indexT i=0;i<Npart();++i)if(probeP[i]==1)tf<<tmi->elapsedTime()<<tab<<velP[i].y<<tab<<load*cos(ex->natFreq(mode)*tmi->elapsedTime())*ex->modeShape(mode,curPosP[i].x)<<nwl;
   serTime.push_back(tmi->elapsedTime());
   for(indexT i=0;i<Npart();++i)if(probeP[i]==1)serVel.push_back(velP[i].y);
...
void initRun(patchPtr&pch,constitPtr&cst,timeIntPtr&tmi,shapePtr&shp){
   const realT beamL=comLineArg("beamL",.1);
   const realT beamH=comLineArg("beamH",.004);
   const realT beamB=comLineArg("beamB",.02);
   const indexT Nmul=comLineArg("Nmul",1);
   const vec3 lo(0,0,0);
   const vec3 hi(beamL,beamH,beamB);
   makeShape(shp,25*Nmul,Nmul,2*Nmul,lo,hi,comLineArg("shape",string("MPM")));
