...
void fillAnnulusRegular(patch& pch, const Vector2& o, const double ri,
                        const double ro, const double ppe, const int Ns) {
// particles are regularly spaced within a rectangular region, whether
// they line up with grid boundaries or not.

   assert(pch.dx == pch.dy);

   // Calculate the number of particles by x and y directions.
   const int nx = int(Pceil(2.0 * ro / (pch.dx / ppe)));
   const int ny = int(Pceil(2.0 * ro / (pch.dy / ppe)));
   ...
   const Vector2 ps(2.0 * ro / double(nx), 2.0 * ro / double(ny));

   // debug
   cerr << "The first particle: " << ps;
   ...
   for (int j = 0; j < ny; j += 1) {
      for (int i = 0; i < nx; i += 1) {
         const Vector2 ns(double(i) + 0.5, double(j) + 0.5);
